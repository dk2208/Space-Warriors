...
void displayRasterText(float x, float y, float z,string stringToDisplay) {
	glRasterPos3f(x, y, z);
	for (int i = 0; i<stringToDisplay.length(); i++) {
		glutBitmapCharacter(GLUT_BITMAP_TIMES_ROMAN_24, stringToDisplay[i]); //helvetica_18,_12
	}
}
...
void introScreen()
{
	...
	glColor3f(1.0, 0.0, 0.0);
	displayRasterText(-625, 490, 0.0, "THAPAR INSTITUTE OF ENGINEERING AND TECHNOLOGY, PATIALA");
	glColor3f(1.0, 1.0, 1.0);
	displayRasterText(-530, 395, 0.0, "DEPARTMENT OF COMPUTER SCIENCE AND ENGINEERING");
	glColor3f(1.0, 1.0, 1.0);
	displayRasterText(-250, 300, 0.0, "ACADEMIC YEAR: 2022-2023");
	glColor3f(0.0, 0.0, 1.0);
	displayRasterText(-180, 205, 0.0, "A MINI PROJECT ON");
	glColor3f(1.0, 0.0, 1.0);
	displayRasterText(-155, 110, 0.0, "SPACE WARRIORS");
	glColor3f(1.0, 0.0, 0.0);
	displayRasterText(-800, -100, 0.0, " SUBMITTED BY :-");
	glColor3f(1.0, 1.0, 1.0);
	displayRasterText(-800, -200, 0.0, "Divyansh Kaushik (102083048)");
	glColor3f(1.0, 0.0, 0.0);
	displayRasterText(500, -100, 0.0, "SUBMITTED TO :-");
	glColor3f(1.0, 1.0, 1.0);
	displayRasterText(500, -200, 0.0, "Ms. Kudrat Aulakh");
	glColor3f(1.0, 1.0, 1.0);
	displayRasterText(-230, -500, 0.0, "Press ENTER to start the game");
	glFlush();
	glutSwapBuffers();
}
...
void startScreenDisplay()
{
	...
	if (mouseX >= 30 && mouseX <= 300 && mouseY >= 70 && mouseY <= 150) {
		glColor3f(0, 0, 1);
		if (mButtonPressed) {
	...
	displayRasterText(-82, 335, 0.4, "Start Game");

	if (mouseX >= 30 && mouseX <= 300 && mouseY >= -80 && mouseY <= -10) {
		glColor3f(0, 0, 1);
		if (mButtonPressed) {
		...
		glColor3f(0, 0, 0);
	displayRasterText(-90, 85, 0.4, "Instructions");

	if (mouseX >= 30 && mouseX <= 300 && mouseY >= -250 && mouseY <= -180) {
		glColor3f(0, 0, 1);
		if (mButtonPressed) {
			mButtonPressed = false;
			exit(0);
		...
		glColor3f(0, 0, 0);
	displayRasterText(-75, -160, 0.4, "    Quit");
	glutPostRedisplay();
}

void backButton() {
	if (mouseX <= 180 && mouseX >= 120 && mouseY >= -530 && mouseY <= -500) {
		glColor3f(0, 0, 1);
		if (mButtonPressed) {
			viewPage = MENU;
			mButtonPressed = false;
	...
	displayRasterText(-70, -600, 0, "Back");
}
...
void instructionsScreenDisplay()
{
	...
	glColor3f(1, 0, 0);
	displayRasterText(-150, 550, 0.4, "INSTRUCTIONS");
	glColor3f(1, 0, 0);
	displayRasterText(-1100, 400, 0.4, "* For PLAYER 1:");
	glColor3f(1, 1, 1);
	displayRasterText(-1050, 300, 0.4, "Key 'w' to move up.");
	displayRasterText(-1050, 200, 0.4, "Key 's' to move down.");
	displayRasterText(-1050, 100, 0.4, "Key 'c' to shoot");
	glColor3f(1, 0, 0);
	displayRasterText(-1100, 0, 0.4, "* For PLAYER 2:");
	glColor3f(1, 1, 1);
	displayRasterText(-1050, -100, 0.4, "Key 'i' to move up.");
	displayRasterText(-1050, -200, 0.4, "Key 'k' to move down.");
	displayRasterText(-1050, -300, 0.4, "Key 'm' to shoot");
	displayRasterText(-1100, -400, 0.4, "* The Objective is to kill your opponent.");
	displayRasterText(-1100, -500, 0.4, "* Each time a player gets shot, LIFE decreases by 5 points.");
	backButton();
}
